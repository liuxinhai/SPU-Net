...
class Generator(object):
    ...
    def __call__(self, inputs):
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
            features = ops_lxh.feature_extraction2(inputs, scope='feature_extraction', is_training=self.is_training, bn_decay=None)
            # print('11111111111111')
            # print(features.get_shape())

            with tf.variable_scope('up_unit', reuse=tf.AUTO_REUSE):
                L = ops_lxh.conv2d(features, 256, [1, 1], padding='VALID', stride=[1, 1], bn=False, is_training=self.is_training, scope='conv0', bn_decay=None)

                num_point = L.get_shape()[1]
                grid = ops_lxh.gen_grid(2)
                ...
                H0 = ops_lxh.up_block_grid(L, tf.concat([grid, lgrid], axis=-1), is_training=self.is_training, bn_decay=None, scope='up_0') #tf.concat([grid, lgrid], axis=-1)

                # grid = tf.tile(grid, [1,2,1,1])
                grid1 = ops_lxh.gen_grid(2) # grid1 = tf.convert_to_tensor(np.array([[-0.2, -0.2], [0.2, -0.2]], dtype=np.float32))
                ...
                H = ops_lxh.up_block_grid(H0, tf.concat([grid1, lgrid1], axis=-1), is_training=self.is_training, bn_decay=None, scope='up_1') #tf.concat([grid, lgrid1], axis=-1)

                # print(H.get_shape())

            coord = ops_lxh.conv2d(H, 64, [1, 1], padding='VALID', stride=[1, 1], bn=False, is_training=self.is_training, scope='fc_layer1', bn_decay=None)

            coord = ops_lxh.conv2d(coord, 3, [1, 1], padding='VALID', stride=[1, 1], bn=False, is_training=self.is_training, scope='fc_layer3', bn_decay=None, activation_fn=None, weight_decay=0.0)
