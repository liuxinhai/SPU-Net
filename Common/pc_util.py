...
def downsample_points_fps(pts, K):
    sampler = FarthestSampler()
    return sampler(pts, K)


def downsample_points_random(pts, K):
    random_list = np.random.choice(pts.shape[0], K, replace=False)
    return pts[random_list, :], random_list
...
class FarthestSampler:
    ...
    def __call__(self, pts, k):
        ...
        farthest_list = np.zeros(k, dtype=np.int32)
        randint = np.random.randint(len(pts))
        farthest_list[0] = randint
        farthest_pts[0] = pts[randint]
        distances = self._calc_distances(farthest_pts[0], pts)
        for i in range(1, k):
            index = np.argmax(distances)
            farthest_list[i] = index
            farthest_pts[i] = pts[index]
            distances = np.minimum(
                distances, self._calc_distances(farthest_pts[i], pts))
        return farthest_pts, farthest_list
